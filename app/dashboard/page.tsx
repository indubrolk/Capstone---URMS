...
import {
    ...
    CheckCircle2, Calendar, ShieldAlert, Clock, BookOpen, PenTool, ArrowRight
} from "lucide-react";
    ...
    return (
        <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="space-y-8">
            <motion.header variants={fadeInUp} className="border-b border-slate-200 dark:border-border pb-6">
                ...
                <p className="text-slate-600 dark:text-foreground/60 mt-2">Welcome back, {profile?.name || "Admin"}. System overview is stable.</p>
            </motion.header>
            ...
            <motion.div variants={fadeInUp} className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-card backdrop-blur-xl rounded-2xl border border-slate-200 dark:border-border p-6 shadow-sm">
                    ...
                    <div className="space-y-4">
                        {[1, 2, 3].map(i => (
                            <div key={i} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-white/5 rounded-xl border border-slate-200 dark:border-border">
                                <div className="flex items-center gap-3">
                                    <div className="bg-cyan-500/10 p-2 rounded-lg text-cyan-400">
                                        <BarChart3 className="w-4 h-4" />
                                    </div>
                                    <p className="text-sm font-semibold text-slate-600 dark:text-foreground/60">New user registration approved</p>
                                </div>
...
function LecturerDashboard({ profile }: { profile: any }) {
    return (
        <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="space-y-8">
            <motion.header variants={fadeInUp} className="bg-gradient-to-r from-blue-900/40 to-cyan-900/20 p-8 rounded-3xl border border-blue-500/20 relative overflow-hidden">
                ...
                <p className="text-blue-500 dark:text-blue-200 mt-2 font-medium relative z-10">Hello, Dr. {profile?.name || "Lecturer"}. You have <span className="font-bold text-cyan-500 dark:text-cyan-400">3 pending approvals</span>.</p>
            </motion.header>
                    ...
                    <div className="flex-grow space-y-3">
                        <div className="p-4 border border-blue-500/20 bg-blue-500/5 rounded-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                            ...
                            <div className="flex gap-2 w-full sm:w-auto">
                                <button className="flex-1 sm:flex-none px-4 py-2 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-border rounded-lg text-xs font-bold text-red-400 hover:bg-slate-100 dark:bg-white/10 transition-colors">Deny</button>
                                <button className="flex-1 sm:flex-none px-4 py-2 bg-cyan-600 text-white rounded-lg text-xs font-bold hover:bg-cyan-500 transition-colors">Approve</button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    return (
        <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="space-y-8">
            <motion.header variants={fadeInUp} className="mb-8">
                ...
                <p className="text-slate-600 dark:text-foreground/60 mt-2 text-lg">Hi {profile?.name || "Student"}, ready to learn?</p>
            </motion.header>
            ...
            <motion.div variants={fadeInUp} className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="bg-gradient-to-br from-cyan-600 to-blue-700 dark:from-cyan-900 dark:to-blue-900 rounded-3xl p-8 text-white shadow-xl shadow-cyan-900/20 relative overflow-hidden border border-cyan-500/30">
                    <div className="absolute -top-10 -right-10 opacity-20 text-cyan-300"><Calendar className="w-48 h-48" /></div>
                ...
                <div className="bg-card backdrop-blur-xl rounded-3xl border border-slate-200 dark:border-border p-8 shadow-sm">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <Link href="/resources" className="flex flex-col items-center justify-center p-6 bg-slate-50 dark:bg-white/5 rounded-2xl hover:bg-slate-100 dark:bg-white/10 transition-colors border border-slate-200 dark:border-border group">
                        ...
                        <Link href="/bookings" className="flex flex-col items-center justify-center p-6 bg-slate-50 dark:bg-white/5 rounded-2xl hover:bg-slate-100 dark:bg-white/10 transition-colors border border-slate-200 dark:border-border group">
...
function MaintenanceDashboard({ profile }: { profile: any }) {
    return (
        <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="space-y-8">
            <motion.header variants={fadeInUp} className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 border-b border-slate-200 dark:border-border pb-6">
                <div>
                    <h1 className="text-3xl font-black text-foreground tracking-tight flex items-center gap-3">
                        <PenTool className="w-8 h-8 text-red-500 dark:text-red-400" /> Operations Hub
                    </h1>
                    <p className="text-slate-600 dark:text-foreground/60 mt-2">Maintenance team active. 2 High priority tickets.</p>
                </div>
                ...
                <div className="bg-red-950/20 border border-red-900/30 rounded-2xl p-6">
                    ...
                    <div className="space-y-3">
                        <div className="bg-slate-50 dark:bg-white/5 p-4 rounded-xl border border-red-500/20 shadow-sm flex justify-between items-center">
                            ...
                            <span className="bg-red-500/20 text-red-500 dark:text-red-400 border border-red-500/30 text-xs font-black uppercase px-2 py-1 rounded">Urgent</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-card border border-slate-200 dark:border-border rounded-2xl p-6 shadow-sm">
                    ...
                    <div className="space-y-3">
                        <div className="bg-slate-50 dark:bg-white/5 p-4 rounded-xl border border-slate-200 dark:border-border flex justify-between items-center">
                            ...
                            <span className="bg-slate-100 dark:bg-white/10 text-slate-600 dark:text-foreground/60 border border-slate-200 dark:border-border text-xs font-black uppercase px-2 py-1 rounded">Routine</span>
                        </div>
                    </div>
                </div>
            </motion.div>
...
function ActionButton({ href, label }: { href: string, label: string }) {
    return (
        <Link href={href} className="bg-card hover:bg-slate-100 dark:bg-white/10 border border-slate-200 dark:border-border hover:border-cyan-500/30 text-foreground font-bold text-sm py-3 px-4 rounded-xl text-center transition-all">
            {label}
        </Link>
    );
